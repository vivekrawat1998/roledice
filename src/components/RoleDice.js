const RoleDice = ({ handleDiceRoll, currentDice, setScore, setCurrentDice, setIsRuleShown}) => {
  return (
    <div className='container w-fit h-fit absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/3'>
      ...
      <button onClick={() => { setScore(0); setCurrentDice(1); }}
      ...
      <br></br>
      <button onClick={() => setIsRuleShown(true)} className='uppercase border-2 border-black px-14 py-2 mx-6 bg-black text-white rounded-md hover:bg-white hover:text-black transition-all duration-1000ms'> 
      ShowRule 
      </button>
    </div>
  );
